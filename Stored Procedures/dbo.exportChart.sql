	...
	BEGIN TRY
		...
		CREATE TABLE #Chart
			(
				...
				,[Member Individual ID]	varchar(100)
				,ChaseID				varchar(50)
				,[PROVIDER TYPE]		varchar(50)
				,REN_TIN				varchar(20)
				,REN_PIN				varchar(20)
				,PID					varchar(20)
				,[CHART NAME]			varchar(200)
		...
		SELECT DISTINCT
				...
				,pm.PIN REN_PIN
				,m.PID
				,SUBSTRING(s.ChaseID,3,5) +'_'+s.EDGEMemberID+'_MR_'+replace(substring(s.ChaseID,charindex(SUBSTRING(s.ChaseID,3,5),s.ChaseID),len(s.ChaseID)),'_','')+'_1of1' AS [CHART NAME]
		FROM	tblSuspect s
