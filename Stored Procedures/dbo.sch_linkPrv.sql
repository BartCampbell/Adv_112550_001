...
BEGIN
		...
		IF NOT EXISTS (SELECT * FROM cacheProviderOffice WITH (NOLOCK) WHERE Project_PK=@project AND ProviderOffice_PK=@office)
			INSERT INTO cacheProviderOffice(Project_PK,ProviderOffice_PK,charts,providers,office_status,contacted,scheduled,extracted,coded) VALUES(@project,@office,0,0,0,0,0,0,0)
